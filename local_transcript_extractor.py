...
def create_simple_summary(text, max_sentences=5):
    ...
    summary_sentences = important_sentences[:min(max_sentences, len(important_sentences))]
    
    return ' '.join(summary_sentences)

def save_results(video_id, youtube_url, language, output_format, content, transcript_data):
    """結果をファイルに保存"""
...
def main():
    ...
    text_file, json_file = save_results(
        video_id, url_input, detected_lang, selected_format, 
