...
def test_filler_removal():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        try:
            ...
            reduction_ratio = (original_length - formatted_length) / original_length * 100
            
            print(f"改善結果:")
            print(f"  文字数: {original_length} → {formatted_length} ({reduction_ratio:+.1f}%)")
            print(f"  期待効果: {case['expected_improvement']}")
            
            # フィラー除去の検証
            fillers_found = []
            common_fillers = ['え〜', 'まあ', 'うん', 'あの〜', 'なんか', 'そう〜', 'um', 'uh', 'you know', 'like', 'well']
