...
def extract():
    try:
        ...
        try:
            ...
            print(
                f"[OK] Transcript fetched successfully: {len(transcript)} segments",
                flush=True,
            )
        except Exception as transcript_error:
            print(
                f"[ERROR] Transcript fetch failed: {str(transcript_error)}", flush=True
            )
            try:
        ...
        return jsonify(
            {"success": False, "error": f"処理中にエラーが発生しました: {str(e)}"}
        )
