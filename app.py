...
def validate_string_input(value, field_name, min_length=1, max_length=255, allow_empty=False):
    """文字列入力の検証"""
    if value is None:
        if not allow_empty:
...
def validate_filename(filename):
    ...
    if file_ext not in allowed_extensions:
        raise ValueError(f'対応していないファイル形式です。対応形式: {", ".join(allowed_extensions)}')
    ...
    def decorated_function(*args, **kwargs):
        ...
        if token.startswith('mock-token-for-development'):
            ...
            if ':' in token:
                try:
                    ...
                    user_data = json.loads(base64.b64decode(user_data_b64).decode('utf-8'))
                    
                    request.user_uid = user_data.get('uid')
                    request.user_email = user_data.get('email') 
                    request.user_name = user_data.get('displayName', request.user_email.split('@')[0] if request.user_email else 'User')
                    
                    # デバッグログでユーザーIDを確認
                    app.logger.info(f"Authenticated user: uid={request.user_uid}, email={request.user_email}")
                except Exception as e:
                    ...
                    app.logger.info(f"Using fallback user: uid={request.user_uid}, email={request.user_email}")
            else:
                # レガシーモックトークンの場合
                request.user_uid = 'mock-user-dev'
                request.user_email = 'dev@example.com'
                request.user_name = 'Development User'
                app.logger.info(f"Using legacy mock user: uid={request.user_uid}, email={request.user_email}")
        ...
        try:
            ...
            if firebase_admin._apps:
                ...
                request.user_name = decoded_token.get('name', decoded_token.get('email', '').split('@')[0])
                app.logger.info(f"✅ Token verified successfully: uid={request.user_uid}, email={request.user_email}")
            else:
            ...
            app.logger.error(f"🚨 Token verification failed: {e} - Token: {token[:20]}...")
            app.logger.error(f"🚨 CRITICAL: Falling back to mock user - THIS CAUSES DATA ISOLATION FAILURE")
    ...
    def decorated_function(*args, **kwargs):
        ...
        if token.startswith('mock-token-for-development'):
            ...
            if ':' in token:
                try:
                    ...
                    request.user = {'uid': 'mock-user', 'email': 'mock@example.com', 'name': 'Mock User'}
        ...
        try:
            ...
            if firebase_admin._apps:
                ...
                # Firebase Admin SDKが利用できない場合はデバッグ用情報を設定
                app.logger.warning("Firebase Admin SDK not available, using mock auth")
                request.user = {'uid': 'mock-user-no-firebase', 'email': 'debug@example.com', 'name': 'Debug User'}
                request.user_uid = 'mock-user-no-firebase'
                request.user_email = 'debug@example.com'
                request.user_name = 'Debug User'
            return f(*args, **kwargs)
    ...
    try:
        ...
        if db:
            ...
            return jsonify({
                ...
                'maxStorageBytes': user_data.get('maxStorageBytes', MAX_USER_STORAGE)
            })
        else:
...
@verify_token
@rate_limit_decorator(max_requests=1000, window=3600)  # アップロードは1時間に1000回まで（テスト用に超大幅緩和）
def create_upload_url():
    ...
    if content_type not in ALLOWED_MIME_TYPES:
        return jsonify({'error': f'Unsupported MIME type. Allowed: {ALLOWED_MIME_TYPES}'}), 400
    ...
    if db is not None:
        try:
            ...
            if user_doc.exists:
                ...
                if current_usage + file_size > user_data.get('maxStorageBytes', MAX_USER_STORAGE):
    ...
    try:
        ...
        try:
            ...
            app.logger.warning(f"Bucket existence check failed: {bucket_error}, proceeding anyway")
...
@verify_token
@rate_limit_decorator(max_requests=1000, window=3600)  # トラック作成は1時間に1000回まで（テスト用に超大幅緩和）
def create_track():
    """トラック作成"""
    data = request.get_json()
    if not data:
    ...
    if storage_client is not None:
        ...
        app.logger.warning("Storage client not available, skipping file existence check")
    ...
    try:
        ...
        if db is not None:
            try:
                ...
                user_ref = db.collection('users').document(request.user_uid)
                user_ref.update({
                    'storageUsed': firestore.Increment(data['sizeBytes'])
                })
                ...
                app.logger.warning(f"Firestore operation failed, falling back to file-only upload: {firestore_error}")
                return jsonify({
                    'trackId': track_id,
            ...
            app.logger.info("Firestore not available, track metadata not saved but file upload successful")
            return jsonify({
                'trackId': track_id,
...
def direct_upload():
    ...
    try:
        ...
        # GCSに直接アップロード
        if storage_client is None:
            app.logger.error(f"Storage client not initialized. STORAGE_BUCKET: {STORAGE_BUCKET}")
            return jsonify({
...
def list_tracks():
    ...
    try:
        ...
        visibility = request.args.get('visibility', 'private')  # デフォルトをprivateに変更（ユーザー自身のトラックのみ）
        search = request.args.get('search', '')
        user_uid = request.args.get('userId', current_user_uid)  # デフォルトで現在のユーザー
        
        app.logger.info(f"🔍 Tracks request: current_user_uid={current_user_uid}, user_uid={user_uid}, limit={limit}, visibility={visibility}, search='{search}', db={db is not None}")
        
        # 🚨 重要：ユーザー認証の検証ログ
        if current_user_uid != user_uid:
            app.logger.warning(f"⚠️ User UID mismatch! current_user_uid={current_user_uid} != user_uid={user_uid}")
        else:
            app.logger.info(f"✅ User UID match confirmed: {current_user_uid}")
        
        # Firestoreが利用できない場合はデモデータを返す
        if db is None:
            app.logger.info("Firestore not available, returning empty tracks array for user: " + current_user_uid)
        ...
        try:
            if visibility == 'public':
                # パブリックの場合：承認済み全トラック
                app.logger.info(f"📊 Building PUBLIC query: visibility='public', status='approved', limit={limit * 2}")
                query = db.collection('tracks').where('visibility', '==', 'public').where('status', '==', 'approved').limit(limit * 2)
            else:
                # プライベートの場合：指定されたユーザーのトラックのみ
                app.logger.info(f"🔒 Building PRIVATE query: uploaderUid='{user_uid}', limit={limit * 2}")
                query = db.collection('tracks').where('uploaderUid', '==', user_uid).limit(limit * 2)
            
            app.logger.info("🔍 Executing Firestore query...")
            docs = query.stream()
            app.logger.info("✅ Firestore query executed successfully")
        except Exception as e:
            app.logger.warning(f"Firestore query failed, returning empty tracks array: {e}")
        ...
        for doc in docs:
            try:
                ...
                app.logger.info(f"📀 Track {track_count}: ID={doc.id[:8]}..., title='{track_title}', uploaderUid='{uploader_uid}', visibility='{track_visibility}'")
        ...
        app.logger.info(f"📊 Query Summary: current_user={current_user_uid}, visibility={visibility}, returned {len(tracks)} tracks (processed {track_count} from Firestore)")
        
        # 🚨 重要：すべてのユーザーに同じトラックが返されているかチェック
        if tracks:
            unique_uploaders = set(track.get('uploaderUid', 'MISSING') for track in tracks)
            app.logger.info(f"🔍 Unique uploaderUids in response: {list(unique_uploaders)}")
            if len(unique_uploaders) > 1 and visibility == 'private':
                app.logger.error(f"🚨 CRITICAL: Multiple uploaderUids found in private mode! Expected only: {user_uid}")
...
def get_play_url(track_id):
    ...
    try:
        ...
        if db is None:
            return jsonify({'error': 'Track playback unavailable (database not configured)'}), 503
        ...
        try:
            ...
            app.logger.warning(f"Signed URL generation failed for playback, using proxy stream: {signed_error}")
...
def stream_track(track_id):
    ...
    try:
        # トラック取得（Firestoreが利用可能な場合のみ）
        if db is None:
            return jsonify({'error': 'Track streaming unavailable (database not configured)'}), 503
        ...
        safe_filename = original_filename.encode('ascii', 'ignore').decode('ascii') or 'audio.mp3'
...
def get_track(track_id):
    ...
    try:
        if db is None:
            return jsonify({'error': 'Track data unavailable (database not configured)'}), 503
        ...
        if track_data.get('uploaderUid') != user_uid:
            return jsonify({'error': 'Access denied. You can only edit your own tracks.'}), 403
...
def update_track(track_id):
    ...
    try:
        if db is None:
            return jsonify({'error': 'Track update unavailable (database not configured)'}), 503
        ...
        if genre is not None:
            ...
            genre = ''
        if genre:
            valid_genres = ['technology', 'business', 'education', 'entertainment', 
                          'news', 'health', 'arts', 'sports', 'science', 'music', 
                          'comedy', 'other']
        ...
        # Firestoreに更新
        app.logger.info(f"🔄 Attempting to update track {track_id} with data: {update_data}")
        
        try:
            db.collection('tracks').document(track_id).update(update_data)
        except Exception as firestore_error:
            app.logger.error(f"Firestore update error: {firestore_error}")
            return jsonify({'error': f'Database update failed: {str(firestore_error)}'}), 500
...
@verify_token
@rate_limit_decorator(max_requests=1000, window=3600)  # 削除は1時間に1000回まで（テスト用に超大幅緩和）
def delete_track(track_id):
    """トラック削除"""
    try:
        # トラック取得と権限確認
        if db is None:
            return jsonify({'error': 'Track deletion unavailable (database not configured)'}), 503
        ...
        if track_data.get('uploaderUid') != request.user_uid:
            return jsonify({'error': 'Permission denied - you can only delete your own tracks'}), 403
        ...
        if storage_client is not None:
            try:
                ...
                if blob.exists():
                    ...
                    app.logger.warning(f"GCS file not found for deletion: {track_data['gcsPath']}")
            except Exception as e:
                app.logger.error(f"Failed to delete GCS file: {e}")
                # GCS削除に失敗してもFirestoreは削除を続行
        else:
            app.logger.warning("Storage client not available, skipping GCS file deletion")
        ...
        try:
            user_library_ref = db.collection('user_libraries').document(request.user_uid)
            user_library_doc = user_library_ref.get()
...
@verify_token
@rate_limit_decorator(max_requests=100, window=3600)  # 破損トラック削除は1時間に100回まで
...
def delete_orphaned_track_impl(track_id):
    ...
    try:
        ...
        if track_exists_in_firestore:
            ...
            is_podcast = track_data.get('isPodcast') or track_data.get('type') == 'podcast'
            
            if is_podcast:
                # ポッドキャストエピソードの場合は、audioUrlがない場合のみ破損とみなす
                is_orphaned = not track_data.get('audioUrl') and not track_data.get('audio_url')
                app.logger.info(f"Podcast episode {track_id} orphaned check: audioUrl={bool(track_data.get('audioUrl'))}, audio_url={bool(track_data.get('audio_url'))}")
            else:
                # 通常トラックの場合は従来の条件
                is_orphaned = (
                    not track_data.get('uploaderUid') or  # アカウントに紐づいていない
                    not track_data.get('audioUrl') or     # 音声URLがない
                    not track_data.get('gcsPath')         # GCSパスがない
                )
            
            if not is_orphaned:
                app.logger.warning(f"Track {track_id} is not orphaned - isPodcast: {is_podcast}, uploaderUid: {track_data.get('uploaderUid')}, audioUrl: {bool(track_data.get('audioUrl'))}, gcsPath: {bool(track_data.get('gcsPath'))}")
                # ポッドキャストの場合でも一時的に削除を許可（テスト用）
                if is_podcast:
                    app.logger.info(f"Allowing deletion of podcast episode {track_id} for cleanup purposes")
                else:
                    return jsonify({'error': 'This track is not orphaned and cannot be deleted using this endpoint'}), 400
            
            app.logger.info(f"Deleting orphaned track: {track_id}, conditions: uploaderUid={track_data.get('uploaderUid')}, audioUrl={bool(track_data.get('audioUrl'))}, gcsPath={bool(track_data.get('gcsPath'))}")
            ...
            if storage_client is not None and track_data.get('gcsPath'):
                try:
                    ...
                    if blob.exists():
                        blob.delete()
                        app.logger.info(f"Deleted GCS file for orphaned track: {track_data['gcsPath']}")
                    else:
                        app.logger.info(f"GCS file not found for orphaned track: {track_data['gcsPath']}")
                except Exception as e:
                    app.logger.warning(f"Failed to delete GCS file for orphaned track: {e}")
                    # GCS削除に失敗してもFirestoreは削除を続行
            ...
            try:
                ...
                return jsonify({'error': 'Failed to delete orphaned track metadata'}), 500
        else:
            # トラックがFirestoreに存在しない場合は、プレイリストとライブラリからのみ削除
            app.logger.info(f"Track {track_id} not found in Firestore but will be removed from playlists and libraries")
            track_data = {}  # 空のデータとして処理
        
        # プレイリストからも削除（全てのプレイリストから削除）
        try:
            # 通常のtracks配列から削除
            playlists = db.collection('playlists').where('tracks', 'array_contains', track_id).get()
            for playlist_doc in playlists:
                ...
                if track_id in tracks:
                    ...
                    app.logger.info(f"Removed orphaned track {track_id} from playlist {playlist_doc.id}")
            ...
            for playlist_doc in all_playlists:
                ...
                for episode in podcast_episodes:
                    if episode.get('episodeId') != track_id:
                        ...
                        removed_count += 1
                        app.logger.info(f"Found orphaned podcast episode to remove: {episode.get('title')} (ID: {track_id})")
                ...
                if removed_count > 0:
                    ...
                    app.logger.info(f"Removed {removed_count} orphaned podcast episode(s) {track_id} from playlist {playlist_doc.id}")
        ...
        try:
            user_libraries = db.collection('user_libraries').where('savedTracks', 'array_contains', track_id).get()
            for user_library_doc in user_libraries:
                ...
                if track_id in saved_tracks:
                    saved_tracks.remove(track_id)
                    user_library_ref.update({'savedTracks': saved_tracks})
                    app.logger.info(f"Removed orphaned track {track_id} from user library {user_library_doc.id}")
        except Exception as e:
            app.logger.warning(f"Failed to remove orphaned track from user libraries: {e}")
        
        return jsonify({
            'message': 'Orphaned track deleted successfully',
            'trackId': track_id
        })
...
def remove_from_library(track_id):
    """個人ライブラリからトラック削除"""
    app.logger.info(f"🔍 Remove from library request: track_id={track_id}, user_uid={request.user_uid}")
    try:
        # Firestoreが利用可能かチェック
        if db is None:
            app.logger.info(f"🔧 Firestore not available, using demo mode for track deletion: {track_id}")
            ...
            app.logger.info(f"✅ Demo mode: Successfully 'removed' track {track_id} from library")
        ...
        # 自分のトラックの場合は完全削除を推奨
        app.logger.info(f"🔍 Owner check: track.uploaderUid='{track_data.get('uploaderUid')}' vs request.user_uid='{request.user_uid}'")
        if track_data.get('uploaderUid') == request.user_uid:
            app.logger.info(f"✅ Owner match detected, returning 400 with permanent_delete message")
            return jsonify({
                'error': 'Use DELETE /api/tracks/{track_id} to permanently delete your own tracks',
                'recommendation': 'permanent_delete'
            }), 400
        else:
            app.logger.info(f"ℹ️ Not owner, proceeding to library removal. Track owner: '{track_data.get('uploaderUid')}'")
        ...
        try:
            ...
            if user_library_doc.exists:
                ...
                if track_id in saved_tracks:
                    ...
                    app.logger.info(f"Removed track {track_id} from user {request.user_uid} library")
                else:
                    return jsonify({'error': 'Track not found in your library'}), 404
            else:
                # ライブラリが存在しない場合は自動初期化
                app.logger.info(f"Personal library not found for user {request.user_uid}, initializing...")
                if ensure_user_library(request.user_uid):
                    # 初期化後は空のライブラリなので、トラックは存在しない
                    return jsonify({'error': 'Track not found in your library (library was just initialized)'}), 404
                else:
                    return jsonify({'error': 'Personal library not found and failed to initialize'}), 500
...
def add_to_library(track_id):
    ...
    try:
        # Firestoreが利用可能かチェック
        if db is None:
            app.logger.info(f"🔧 Firestore not available, using demo mode for track addition: {track_id}")
            ...
            app.logger.info(f"✅ Demo mode: Successfully 'added' track {track_id} to library")
        ...
        try:
            ...
            if user_library_doc.exists:
                ...
                if track_id not in saved_tracks:
                    ...
                    app.logger.info(f"Added track {track_id} to user {request.user_uid} library")
                else:
                ...
                app.logger.info(f"Created library and added track {track_id} for user {request.user_uid}")
    ...
    try:
        ...
        for user_doc in all_users_docs:
            ...
            if (query in display_name or 
                ...
                # そのユーザーのトラック数と総時間を計算
                user_tracks = db.collection('tracks').where('uploaderUid', '==', user_id).stream()
                track_count = 0
                total_duration = 0
                ...
                filtered_users.append({
                    ...
                    'joinedDate': user_data.get('createdAt', datetime.utcnow()).strftime('%Y-%m-%d') if isinstance(user_data.get('createdAt'), datetime) else user_data.get('createdAt', '')
                })
        
        app.logger.info(f"Firestore user search query: '{query}', found {len(filtered_users)} users")
...
def clear_rate_limits():
    ...
    try:
        ...
        app.logger.info(f"Rate limits cleared by admin (cleared {cleared_count} entries)")
        return jsonify({
...
def get_user_profile(user_id):
    ...
    try:
        ...
        if db is None:
            app.logger.warning("🟡 [DEBUG] Firestore database not configured, returning mock profile")
            # 開発環境向けのモックプロフィールを返す
            mock_profile = {
                'userId': user_id,
                'displayName': user_id.replace('user_', '').replace('_dev', '').replace('_', '@'),
            ...
            app.logger.info(f"✅ [DEBUG] Mock profile generated successfully for {user_id}")
            return jsonify({
        ...
        app.logger.info(f"🔍 [DEBUG] Attempting to get user profile document from Firestore")
        ...
        app.logger.info(f"🔍 [DEBUG] Retrieved document, exists: {user_profile_doc.exists}")
        
        if not user_profile_doc.exists:
            app.logger.warning(f"🟡 [DEBUG] User profile not found in Firestore: {user_id}, creating mock profile")
        ...
        app.logger.info(f"🔍 [DEBUG] User data retrieved: {list(user_data.keys()) if user_data else 'None'}")
        ...
        for track_doc in user_tracks_docs:
            ...
            if track_data.get('visibility', 'private') == 'public':
                ...
                if not isinstance(created_at, datetime):
                    try:
                        created_at = datetime.fromisoformat(str(created_at)) if created_at else datetime.utcnow()
                    except:
        ...
        profile = {
            ...
            'followersCount': user_data.get('followersCount', 0),
            'joinedDate': user_data.get('createdAt', datetime.utcnow()).strftime('%Y-%m-%d') if isinstance(user_data.get('createdAt'), datetime) else str(user_data.get('createdAt', '')),
            'avatarUrl': user_data.get('avatarUrl', ''),
            'tracks': tracks
        }
        
        app.logger.info(f"✅ [DEBUG] Retrieved Firestore profile for user: {user_id}, tracks: {track_count}")
...
def create_or_update_user_profile():
    ...
    try:
        ...
        display_name = data.get('displayName', user_email.split('@')[0] if user_email else 'Unknown User')
        bio = data.get('bio', '')
...
def get_user_playlists():
    ...
    try:
        # デバッグ用ログ追加
        app.logger.info(f"🔍 Getting playlists for user: {getattr(request, 'user_uid', 'NO_UID')}")
        
        if db is None:
            app.logger.warning("⚠️ Database not configured - returning 503")
            return jsonify({'error': 'Playlist service unavailable (database not configured)'}), 503
        ...
        return jsonify({'error': 'Failed to retrieve playlists', 'details': str(e)}), 500
...
def create_playlist():
    ...
    try:
        ...
        if len(name) > 100:
            return jsonify({'error': 'Playlist name must be 100 characters or less'}), 400
...
def add_podcast_episode_to_playlist(playlist_id):
    ...
    try:
        ...
        if playlist_data.get('creatorUid') != user_uid:
            return jsonify({'error': 'Access denied. You can only modify your own playlists.'}), 403
        ...
        if episode_id in current_tracks or any(ep.get('episodeId') == episode_id for ep in podcast_episodes):
        ...
        app.logger.info(f"✅ Podcast episode {episode_id} added to playlist {playlist_id} by user {user_uid}")
        ...
        app.logger.error(f"❌ Failed to add podcast episode to playlist: {e}", exc_info=True)
        return jsonify({'error': 'Failed to add episode to playlist', 'details': str(e)}), 500
...
def clear_podcast_episodes_from_playlist(playlist_id):
    ...
    try:
        ...
        app.logger.info(f"✅ Cleared {removed_count} podcast episodes from playlist {playlist_id} by user {user_uid}")
...
@app.route('/api/playlists/<playlist_id>/podcast-episodes/<episode_id>', methods=['DELETE'])
@verify_token
def remove_podcast_episode_from_playlist(playlist_id, episode_id):
    ...
    try:
        ...
        if not playlist_doc.exists:
            ...
            if not playlist_doc.exists:
                return jsonify({'error': 'Playlist not found in either collection'}), 404
        ...
        for i, episode in enumerate(current_podcast_episodes):
            if episode.get('trackId') == episode_id or episode.get('episodeId') == episode_id:
        ...
        app.logger.info(f"✅ Podcast episode {episode_id} removed from playlist {playlist_id} by user {user_uid}")
...
def maintenance_clear_invalid_podcasts():
    ...
    try:
        ...
        maintenance_key = request.json.get('maintenance_key') if request.is_json else None
        ...
        for playlist_doc in playlists:
            ...
            if removed_count > 0:
                ...
                app.logger.info(f"🧹 Maintenance: Removed {removed_count} invalid podcast episodes from playlist {playlist_id}")
        ...
        return jsonify({'error': 'Maintenance operation failed', 'details': str(e)}), 500
...
def debug_playlist_contents(playlist_id):
    ...
    try:
        ...
        return jsonify({'error': 'Failed to get playlist debug info', 'details': str(e)}), 500
...
def debug_tracks():
    ...
    try:
            ...
            try:
                ...
                for doc in docs:
                    ...
                    collection_data.append({
                        ...
                        'uploaderName': track_data.get('uploader_name', track_data.get('uploaderName', 'Unknown')),
        ...
        return jsonify({'error': 'Failed to get tracks debug info', 'details': str(e)}), 500
...
def get_tracks_by_ids():
    ...
    try:
        ...
        for track_id in track_ids:
            try:
                ...
                if doc.exists:
                    ...
                    tracks.append(track_data)
                    app.logger.info(f"✅ Found track: {track_id} - {track_data.get('title', 'Unknown')}")
                else:
        ...
        app.logger.info(f"📊 Returning {len(tracks)} tracks from {len(track_ids)} requested IDs")
...
def delete_playlist(playlist_id):
    ...
    try:
        ...
        playlist_data = playlist_doc.to_dict()
        if playlist_data.get('creatorUid') != user_uid:
            return jsonify({'error': 'Access denied. You can only delete your own playlists.'}), 403
...
def add_track_to_playlist(playlist_id):
    ...
    try:
        ...
        update_data = {
            ...
            'totalDuration': playlist_data.get('totalDuration', 0) + track_data.get('duration', 0),
            'updatedAt': datetime.utcnow()
        }
        
        db.collection('playlists').document(playlist_id).update(update_data)
        
        app.logger.info(f"✅ Track {track_id} added to playlist {playlist_id} by user {user_uid}")
...
def remove_track_from_playlist(playlist_id, track_id):
    ...
    try:
        ...
        app.logger.info(f"✅ Track {track_id} removed from playlist {playlist_id} by user {user_uid}")
...
def search_episodes_by_keywords(keywords, max_results=200):
    ...
    for podcast_id, podcast_info in JAPANESE_TECH_PODCASTS.items():
        try:
            ...
            # RSSフィードを取得
            headers = {'User-Agent': 'Mozilla/5.0 (compatible; AIIV-Podcast-Search/1.0)'}
            response = requests.get(podcast_info['rss_url'], headers=headers, timeout=10)
            ...
            app.logger.info(f"📊 Found {len(feed.entries)} episodes in {podcast_info['name']}")
            ...
            for entry in feed.entries[:50]:  # 最新50件をチェック
                ...
                if keyword_matches or not keywords:  # キーワードが一致するか、キーワード指定なしの場合
                    # エピソード情報を構築
                    episode = {
                        'id': f"{podcast_id}_{hash(entry.get('link', ''))}",
                        'title': entry.get('title', 'Untitled'),
                        'description': entry.get('description', '')[:500] + ('...' if len(entry.get('description', '')) > 500 else ''),
                        'summary': entry.get('summary', '')[:300] + ('...' if len(entry.get('summary', '')) > 300 else ''),
    ...
    # 日付でソート（新しい順）
    results.sort(key=lambda x: x.get('published_parsed') or (0, 0, 0, 0, 0, 0), reverse=True)
...
def search_podcasts():
    ...
    try:
        ...
        app.logger.info(f"🔍 Podcast search: query='{query}', category='{category}', limit={limit}")
...
def subscribe_to_podcast():
    ...
    try:
        ...
        existing = subscriptions_ref.where('user_uid', '==', user_uid)\
...
def get_user_subscriptions():
    ...
    try:
        ...
        subscriptions_ref = db.collection('podcast_subscriptions')
        subscriptions = subscriptions_ref.where('user_uid', '==', user_uid)\
                                       .where('enabled', '==', True)\
        ...
        for doc in subscriptions:
            ...
            subscription_info = {
                ...
                'subscribed_at': data['subscribed_at'].isoformat() if data.get('subscribed_at') else None,
                'last_fetched': data['last_fetched'].isoformat() if data.get('last_fetched') else None,
...
def get_subscription_episodes(subscription_id):
    ...
    try:
        ...
        return jsonify({
            'podcast_info': subscription_data['podcast_info'],
            'episodes': episodes
        })
...
def add_to_favorites():
    ...
    try:
        ...
        existing = favorites_ref.where('user_uid', '==', user_uid)\
...
def get_favorites():
    ...
    try:
        ...
        favorites_ref = db.collection('favorites')
        favorites = favorites_ref.where('user_uid', '==', user_uid)\
        ...
        for doc in favorites:
            ...
            favorite_info = {
                ...
                'added_at': data['added_at'].isoformat() if data.get('added_at') else None
            }
...
def record_playback():
    ...
    try:
        ...
        existing = history_ref.where('user_uid', '==', user_uid)\
                             .where('episode_id', '==', episode_id)\
                             .limit(1).get()
        ...
        playback_data = {
            ...
            'progress': progress,
            'duration': duration,
            'completed': progress >= duration * 0.9 if duration > 0 else False,  # 90%以上で完了とみなす
...
def get_playback_history():
    ...
    try:
        ...
        history_ref = db.collection('playback_history')
        history = history_ref.where('user_uid', '==', user_uid)\
        ...
        for doc in history:
            ...
            history_info = {
                ...
                'first_played': data['first_played'].isoformat() if data.get('first_played') else None,
                'last_played': data['last_played'].isoformat() if data.get('last_played') else None
            }
...
def download_podcast_episode():
    ...
    try:
        ...
        # 音声ファイルをダウンロード
        headers = {'User-Agent': 'Mozilla/5.0 (compatible; AIIV-Podcast-Downloader/1.0)'}
...
def debug_tracks_count():
    ...
    try:
        ...
        # 現在のユーザーのトラック数をカウント
        current_user_uid = request.user_uid
        user_tracks = db.collection('tracks').where('uploaderUid', '==', current_user_uid).get()
...
def debug_auth_status():
    ...
    try:
        ...
        # 基本的な情報を返す
        return jsonify({
            'authHeader': auth_header[:50] + '...' if len(auth_header) > 50 else auth_header,
...
def clear_orphaned_tracks():
    ...
    try:
        ...
        if orphaned_tracks:
            user_library_ref.update({
                'savedTracks': valid_tracks,
                'updatedAt': datetime.utcnow()
            })
            
            app.logger.info(f"Removed {len(orphaned_tracks)} orphaned track references: {orphaned_tracks}")
...
def clear_orphaned_tracks_direct(user_uid):
    ...
    try:
        ...
        return jsonify({'error': 'Failed to clear orphaned tracks', 'userUid': user_uid}), 500
...
def add_to_podcast_library():
    ...
    try:
        ...
        app.logger.info(f"Added episode {episode_id} to podcast library for user {user_uid}")
...
def get_podcast_library():
    ...
    try:
        user_uid = request.user_uid
        view = request.args.get('view', 'episodes')  # episodes, podcasts, playlists, settings
        ...
        app.logger.error(f"Failed to get podcast library for user {request.user_uid}: {e}")
...
def remove_from_podcast_library(episode_id):
    ...
    try:
        ...
        app.logger.info(f"Removed episode {episode_id} from podcast library for user {user_uid}")
...
def get_podcast_playlists():
    ...
    try:
        ...
        playlists_ref = db.collection('podcast_playlists').where('user_uid', '==', user_uid)
        ...
        app.logger.error(f"Failed to get podcast playlists for user {request.user_uid}: {e}")
...
def create_podcast_playlist():
    ...
    try:
        ...
        app.logger.error(f"Failed to create podcast playlist for user {request.user_uid}: {e}")
...
def add_episode_to_podcast_playlist(playlist_id):
    ...
    try:
        ...
        return jsonify({
            'message': 'Episode added to podcast playlist successfully'
        })
        
    except Exception as e:
        app.logger.error(f"Failed to add episode to podcast playlist {playlist_id}: {e}")
        return jsonify({'error': 'Failed to add episode to playlist'}), 500

@app.route('/api/podcast-playlists/<playlist_id>/episodes/<episode_id>', methods=['DELETE'])
@verify_token
def remove_episode_from_podcast_playlist(playlist_id, episode_id):
    ...
    try:
        ...
        app.logger.error(f"Failed to remove episode from podcast playlist {playlist_id}: {e}")
        return jsonify({'error': 'Failed to remove episode from playlist'}), 500
