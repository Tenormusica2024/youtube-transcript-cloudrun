                ...
                try:
                    for transcript_info in transcript_list:
                        if transcript_info.language_code == try_lang:
                            ...
                            print(
                                f"{try_lang} 字幕抽出成功: {len(transcript)} セグメント"
                            )
...
def send_to_api_for_formatting(transcript_text, api_url, lang="ja"):
    ...
    try:
        ...
        # 字幕テキストを送信
        payload = {"transcript_text": transcript_text, "lang": lang, "format": "txt"}
...
def save_results(
    video_id, youtube_url, language, formatted_content, summary, transcript_data
):
    """結果をファイルに保存"""
...
def main():
    ...
    text_file, json_file = save_results(
        video_id, url_input, detected_lang, formatted_text, summary, transcript
    )
    ...
    print("-" * 60)
    preview = (
        formatted_text[:300] + "..." if len(formatted_text) > 300 else formatted_text
    )
    print(preview)
    print("-" * 60)
