"""
YouTube Transcript Extractor - スマホ対応版（Port 5001）
フル機能版：AI要約、QRコード、YouTube Redテーマ
"""
            ...
            for tunnel in tunnels:
                if tunnel.get("proto") == "https":
                    ...
                    if public_url:
                        ...
                        return public_url
        logger.warning("ngrok API応答なし - デフォルト値を使用")
        return "Not available"
    except Exception as e:
        logger.warning(f"ngrok URL動的取得失敗: {e}")
        return "Not available"
...
def generate_qr():
    ...
    try:
        ...
        default_url = get_ngrok_url() or f"http://127.0.0.1:{PORT}"
...
def health_check():
    ...
    health_data = {
        ...
        "version": "2.0.0-port5001-full",
        "services": {
            "youtube_api": "operational",
            "ai_formatting": "operational" if get_ai_api_key() else "degraded",
            "ngrok_tunnel": (
                "operational" if get_ngrok_url() != "Not available" else "fallback"
            ),
        },
        "performance": {
...
def detailed_status():
    ...
    try:
        ...
        ngrok_status = "active" if get_ngrok_url() != "Not available" else "static"
