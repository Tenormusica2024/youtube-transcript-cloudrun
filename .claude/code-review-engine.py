...
class CodeReviewEngine:
    ...
    def run_comprehensive_review(self) -> Dict[str, Any]:
        ...
        print(
            f"[Files] Reviewing {len(changed_files)} files: {', '.join(changed_files[:5])}{'...' if len(changed_files) > 5 else ''}"
        )
    ...
    def run_security_analysis(self, files: List[str]):
        ...
        for file_path in files:
            ...
            injections_found = self.scan_for_injections(
                content, patterns.get("injections", {})
            )
            self.security_issues.extend(injections_found)

            # Check for crypto issues
            crypto_issues = self.scan_for_crypto_issues(
                content, patterns.get("crypto_issues", {})
            )
            self.security_issues.extend(crypto_issues)

        critical_issues = [
            issue for issue in self.security_issues if issue["severity"] == "critical"
        ]
        if critical_issues:
        ...
        print(
            f"[Security] {len(self.security_issues)} issues found ({len(critical_issues)} critical)"
        )
    ...
    def run_performance_analysis(self, files: List[str]):
        ...
        for file_path in files:
            ...
            complexity_issues = self.analyze_complexity(
                file_path, content, config.get("complexity_thresholds", {})
            )
        ...
        print(
            f"[Performance] {len(self.performance_issues)} issues found ({len(high_issues)} high-priority)"
        )

    def analyze_complexity(
        self, file_path: str, content: str, thresholds: Dict
    ) -> List[Dict]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    if complexity > thresholds.get("cyclomatic_complexity", {}).get(
                        "error", 20
                    ):
                        issues.append(
                            {
                    ...
                    issues.append(
                        {
                            ...
                            "category": category,
                            "severity": "medium",
                            "message": pattern_info.get(
                                "issue", "Performance issue detected"
                            ),
                            "line": content[: match.start()].count("\n") + 1,
                            "fix_suggestion": pattern_info.get(
    ...
    def run_coverage_analysis(self, files: List[str]):
        ...
        try:
            ...
            if coverage_result.returncode == 0:
                ...
                print(
                    f"[Coverage] {total_coverage:.1f}% ({len(self.coverage_issues)} issues)"
                )
...
"""
        ...
        if self.performance_issues:
            ...
            for issue in self.performance_issues[:5]:
                report += f"- **{issue['type'].title()}**: {issue['message']} (Line {issue.get('line', 'N/A')})\n"
                report += (
                    f"  *Fix*: {issue.get('fix_suggestion', 'Consider optimization')}\n"
                )
