...
REQUIRED_ENV_VARS = {
    ...
    "ALLOWED_ORIGINS": {
        ...
        "validation": lambda x: all(
            origin.strip().startswith(("http://", "https://"))
            for origin in x.split(",")
        ),
...
class EnvValidator:
    ...
    def __init__(self, env_file_path=".env"):
        ...
        if self.env_file.exists():
            ...
            self.issues.append(
                f"⚠️  .envファイルが見つかりません: {self.env_file.absolute()}"
            )
    ...
    def _validate_single_var(self, var_name, config):
        ...
        if config["required"] and not current_value:
            if config["default"]:
                ...
                self.issues.append(
                    f"❌ {var_name}: 必須環境変数が未設定（デフォルト値なし）"
                )
            return
        ...
        if current_value and config["validation"]:
            try:
                if not config["validation"](current_value):
                    ...
                    self.issues.append(
                        f"   現在値: {current_value[:20]}..."
                        if len(current_value) > 20
                        else f"   現在値: {current_value}"
                    )
                else:
                    print(f"✅ {var_name}: OK")
