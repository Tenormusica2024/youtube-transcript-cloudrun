    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                # 重要フィラーの除去状況詳細確認
                critical_fillers = ['ガスも', 'うん', 'あ、', 'で、', 'あれか', 'ちゃんと', 'ですね', 'って話', 'によって', 'とですね']
                ...
                for filler in critical_fillers:
                    ...
                    if original_count > 0:
                        ...
                        print(f"  {filler}: {original_count} → {formatted_count} ({status})")
                
                print()
                
                if total_original > 0:
                    overall_removal_rate = ((total_original - total_formatted) / total_original) * 100
                    print(f"総合除去率: {overall_removal_rate:.1f}% ({total_original - total_formatted}/{total_original})")
                    print(f"成功フィラー数: {successful_removals}/{len([f for f in critical_fillers if original.count(f) > 0])}")
                    ...
                    if overall_removal_rate >= 80 and change_rate >= 3:
                        ...
                        print(f"部分的改善: {overall_removal_rate:.1f}%除去, {change_rate:.1f}%短縮")
                        return False
                    else:
                        print()
                        print("フィラー除去機能 未動作")
